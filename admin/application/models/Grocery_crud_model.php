...
class Grocery_crud_model  extends CI_Model  {
    ...
    function get_relation_array($field_name , $related_table , $related_field_title, $where_clause, $order_by, $limit = null, $search_like = null)
    {
        ...
        // if($related_field_title == "{class} - {school_id}"){
        ...
        if(strpos($related_field_title, "{school_id")){

            

            foreach ($relation_array as $key_r_a => &$value_r_a) {
               
                $split = explode("-", $value_r_a);
                 $this->db->select("name, branch");

                $s_id = str_replace('%26nbsp%3B', '', urlencode($split[1]));
                

                //echo $s_id;
                 $this->db->where("school_id", $s_id);
                 $this->db->limit(1);
                 $school_query = $this->db->get("Schools");
                 $school = $school_query->row();
              
                 $value_r_a = $split[0]." - ". $school->name." - ".  $school->branch;
            }
        }
